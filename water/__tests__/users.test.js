...
describe('user service layer testing', () => {
  ...
  test('can log into that user', async () => {
    const response = await serviceHandler.userService.loginWithEmailAndPassword(
      {
        email: 'user@123.com',
        password: 'skiing'
      }
  ...
  test("can get that user by it's id", async () => {
    const response = await serviceHandler.userService.getUserFromId({ userId })
  ...
  test('can check completed on an adventure', async () => {
    ...
    const afterUser = await serviceHandler.userService.getUserFromId({ userId })
