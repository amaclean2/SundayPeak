...
const createStatements = [
  ...
  `CREATE TABLE ski( id INT AUTO_INCREMENT, avg_angle FLOAT, max_angle FLOAT, approach_distance VARCHAR(50), aspect VARCHAR(3), difficulty INT, summit_elevation VARCHAR(50), base_elevation VARCHAR(50), exposure INT, gear VARCHAR(50), season VARCHAR(100), PRIMARY KEY(id) );`,
  ...
  `CREATE TABLE conversation_interactions( user_id INT, conversation_id INT, PRIMARY KEY(user_id, conversation_id), FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE, FOREIGN KEY(conversation_id) REFERENCES conversations(id) ON DELETE CASCADE );`,
  `CREATE TABLE messages( id INT AUTO_INCREMENT, conversation_id INT, sender_id INT, message_text TEXT, data_reference VARCHAR(255), PRIMARY KEY(id), FOREIGN KEY(conversation_id) REFERENCES conversations(id) ON DELETE CASCADE, FOREIGN KEY(sender_id) REFERENCES users(id) ON DELETE CASCADE );`
]
