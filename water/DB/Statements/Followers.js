...
const getFriendsStatement = `SELECT
CONCAT(l.first_name, ' ', l.last_name) AS leader_display_name,
l.id AS leader_id,
l.email AS leader_email,
CONCAT(m.first_name, ' ', m.last_name) AS follower_display_name,
m.email AS follower_email,
m.id AS follower_id
FROM friends AS f
INNER JOIN users AS l ON f.leader_id = l.id
INNER JOIN users AS m ON f.follower_id = m.id
