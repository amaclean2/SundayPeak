...
describe('GET /loggedIn', () => {
  ...
  describe('getting data from another user', () => {
    it('should ensure an access token in the authorization header is checked for', async () => {
      const response = await request(app).get(`${USERS_PREFIX}${route}`)

      expect(response?.body?.error).toBeDefined()
      expect(response.body.error.message).toBe(
        errorTexts.notLoggedIn.messageText
      )
    ...
    it('should ensure only valid access tokens are authenticated', async () => {
      ...
      expect(response?.body?.error).toBeDefined()
      expect(response.body.error.message).toBe(
        errorTexts.JsonWebTokenError.messageText
      )
      expect(response.statusCode).toBe(UNAUTHORIZED)
    })
    ...
    it('should ensure all correct responses are formatted as JSON and a 2xx status code', async () => {
      ...
      expect([SUCCESS, CREATED, ACCEPTED].includes(response.statusCode)).toBe(
        true
      )
    ...
    it('should ensure a mapbox token is sent back with the user', async () => {
      const response = await request(app)
        .get(`${USERS_PREFIX}${route}`)
        .set({ authorization: `Bearer ${token}` })

      expect(response?.body?.data).toBeDefined()
      expect(response.body.data.mapbox_token).toBeDefined()
      expect(response.body.data.mapbox_token.length).toBeGreaterThan(0)
    })
  })
})
