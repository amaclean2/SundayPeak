...
// ticks
const selectTicksByAdventureStatement = `SELECT t.id, u.first_name, u.last_name, u.email, t.creator_id FROM ticks AS t
INNER JOIN users AS u ON t.creator_id = u.id
INNER JOIN adventures AS a ON t.adventure_id = a.id WHERE t.adventure_id = ?`;
const selectTicksByUserStatement = `SELECT t.id, t.creator_id, a.adventure_name, a.adventure_type, a.difficulty, a.nearest_city FROM ticks AS t
INNER JOIN users as u ON t.creator_id = u.id
...
// activities
const selectActivitiesByAdventureStatement = `SELECT ac.id, u.first_name, u.last_name, u.email, ac.creator_id FROM activities AS ac
INNER JOIN users AS u ON ac.creator_id = u.id
INNER JOIN adventures AS a ON ac.adventure_id = a.id WHERE ac.adventure_id = ?`;
const selectActivitiesByUserStatement = `SELECT ac.id, ac.creator_id, a.adventure_name, a.adventure_type, a.difficulty, a.nearest_city FROM activities AS ac
INNER JOIN users AS u ON ac.creator_id = u.id
...
const countActivitiesStatement = "SELECT COUNT(id) FROM activities WHERE creator_id = ?";
...
const getFollowersCountStatement = `SELECT COUNT(id) FROM followers WHERE leader_id = ?`;
...
const getFollowingCountStatement = `SELECT COUNT(id) FROM followers WHERE follower_id = ?`;
